...
- **Backend:** REST and the voice socket need `--dart-define=VOICEOPS_API_URL=…` (`lib/core/config/backend_config.dart`). `voiceSessionProvider` is the only WebSocket owner. Tests that tap push-to-talk or open the Map tab add `offlineOverrides()` from `test/fake_voice.dart`, which fakes the socket, mic, speaker, REST, GPS and map tiles.
