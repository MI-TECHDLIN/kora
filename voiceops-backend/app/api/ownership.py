...
async def require_owned_shift(
    ...
    *,
    ...
    if allow_mock_id and not is_valid_uuid(shift_id) and settings.mock_delivery_ids_enabled:
...
async def require_owned_delivery(
    ...
    if not is_valid_uuid(delivery_id) and settings.mock_delivery_ids_enabled:
        # Mock/demo deliveries are intentionally not database-backed, so there is no
        # ownership chain to verify. This bypass is never active in production.
        return None
