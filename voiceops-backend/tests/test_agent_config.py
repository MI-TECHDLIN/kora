...
def test_first_greeting_is_personal_polite_and_lively():
    greeting = get_agent_greeting("Elena", question_index=0)

    assert greeting.startswith("Hello, Elena.")
...
def test_first_greeting_question_can_vary_deterministically():
    first = get_agent_greeting("Elena", question_index=0)
    second = get_agent_greeting("Elena", question_index=1)
...
def test_every_reply_is_instructed_to_be_calm_and_friendly():
    prompt = get_system_prompt("Elena")
