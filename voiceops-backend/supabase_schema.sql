...
-- Order dispatch (2026-09-13). Idempotent: run it on an existing database too.
-- A logistics platform's new order is a deliveries row with shift_id NULL and status
-- 'offered' (waiting on one driver's answer) or 'unassigned' (nobody took it yet) until a
-- driver accepts it, which sets shift_id and status 'pending' (docs/contracts/interface.md §3).
-- The RLS policy on deliveries already hides these rows from drivers: they belong to no shift.
